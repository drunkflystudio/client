...
if(NOT IS_SECOND_RUN)
    ...
    set(IS_SECOND_RUN TRUE CACHE INTERNAL "" FORCE)
else()
...
foreach(it ${COMPONENTS_QT})
    find_package(Qt${QT_VERSION_MAJOR} COMPONENTS ${it} REQUIRED)
endforeach()
...
if (APPLE AND NOT DEFINED PLATFORM_NAME)
    ...
    #
    if (IS_FIRST_RUN)
        message(STATUS "Querying XCode for SDK root")
    endif()
    ...
    if (IS_FIRST_RUN)
        message(STATUS "Set sysroot for ${PLATFORM_NAME} to ${SDKROOT}")
    endif()
    # Setting C Compiler
...
if (WASM_TARGET)
    if (IS_FIRST_RUN)
        #message(STATUS "Cross-building for Webassembly")
    endif()
    function (cute_add_executable TARGET_NAME)
        ...
        set(TARGET_DIR "${CMAKE_BINARY_DIR}/Binaries")
        ...
        add_custom_command(TARGET
                        ...
                        COMMAND
                            ${CMAKE_COMMAND} -E copy
                                "${QT_SDK_DIR}/plugins/platforms/qtloader.js"
                                "${TARGET_DIR}"
                        COMMAND
                            ${CMAKE_COMMAND} -E copy
                                "${QT_SDK_DIR}/plugins/platforms/qtlogo.svg"
                                "${TARGET_DIR}"
                        COMMAND
                            ${CMAKE_COMMAND} -E copy
                                "${CMAKE_CURRENT_LIST_DIR}/res/drunkfly.ico"
                                "${TARGET_DIR}/favicon.ico"
                        COMMENT
                            "Added qtloader.js, qtlogo.svg and favicon.ico to project")
        set(APPNAME "Application")
        set(APPEXPORTNAME "application")
        set_target_properties(${TARGET_NAME} PROPERTIES OUTPUT_NAME "${APPNAME}")
        set_target_properties(${TARGET_NAME} PROPERTIES RUNTIME_OUTPUT_DIRECTORY "${TARGET_DIR}")
        set_target_properties(${TARGET_NAME} PROPERTIES QT_WASM_EXPORT_NAME "${APPEXPORTNAME}")
        configure_file("${QT_SDK_DIR}/plugins/platforms/wasm_shell.html" "${TARGET_DIR}/index.html" @ONLY)
        ...
        foreach(it ${COMPONENTS_QT})
            target_link_libraries(${TARGET_NAME} PRIVATE "Qt${QT_VERSION_MAJOR}::${it}")
        endforeach()
    endfunction ()
...
elseif (APPLE_MACOSX OR APPLE_IOS_TARGET)
    if (${PLATFORM_NAME} STREQUAL macosx
        ...
        function (cute_add_executable TARGET_NAME)
            ...
            if (MACOSX_BUNDLE)
                ...
                if(${PLATFORM_NAME} STREQUAL iphoneos OR ${PLATFORM_NAME} STREQUAL iphonesimulator)
                    ...
                    if ("${MACOSX_BUNDLE_INFO_PLIST}" STREQUAL "")
                        message(STATUS "Using ${DEFAULT_IOS_PLIST_FILE} as template plist file for iOS.")
                    ...
                    if (NOT EXISTS "${COMPILED_SPLASH_SCREEN}")
                        ...
                        if(IS_FIRST_RUN)
                            message(STATUS "Generated compiled splash screen storyboard file ${COMPILED_SPLASH_SCREEN} from ${SPLASH_SCREEN}")
                        endif()
            ...
            if (MACOSX_BUNDLE)
                if (APPLE_IOS_DEVICE OR APPLE_IOS_SIMULATOR)
                    ...
                    add_custom_command(TARGET
                                       ...
                                       COMMAND
                                       ${QT_SDK_DIR}/bin/macdeployqt $<TARGET_BUNDLE_DIR:${TARGET_NAME}>
                                            #-qmldir=${CMAKE_CURRENT_SOURCE_DIR}
                                            ${APPLE_CODESIGN_CMD_OPT} -appstore-compliant)
                endif()
            endif()
            foreach(it ${COMPONENTS_QT})
                target_link_libraries(${TARGET_NAME} PRIVATE "Qt${QT_VERSION_MAJOR}::${it}")
            endforeach()
            if(APPLE_MACOSX)
                target_sources(${TARGET_NAME} PRIVATE "${CMAKE_CURRENT_LIST_DIR}/Apple/macOS/AppIcon.icns")
                set_source_files_properties("${CMAKE_CURRENT_LIST_DIR}/Apple/macOS/AppIcon.icns"
                    PROPERTIES MACOSX_PACKAGE_LOCATION "Resources")
            elseif(APPLE_IOS_DEVICE OR APPLE_IOS_SIMULATOR)
                target_sources(Client PRIVATE "${CMAKE_CURRENT_LIST_DIR}/Apple/iOS/AppIcon.icns")
                set_source_files_properties("${CMAKE_CURRENT_LIST_DIR}/Apple/iOS/AppIcon.icns"
                    PROPERTIES MACOSX_PACKAGE_LOCATION "Resources")
            endif()
        endfunction ()
...
elseif (ANDROID_TARGET)
    ...
    function (cute_add_executable TARGET_NAME)
        ...
        if (NOT AndroidMultiAbiBuild)
            message(STATUS "Cross-compiling for Android with multi-abi support. Building ${TARGET_NAME} for the following abis: ${ANDROID_ABIS}.")
            ...
            if(DEFINED ANDROID_SDK_DIR_armeabi-v7a)
                ...
                list(APPEND QT_EXTRA_PREFIX_DIRS "\"${ANDROID_SDK_DIR_armeabi-v7a}/lib\"")
            endif()
            if(DEFINED ANDROID_SDK_DIR_arm64-v8a)
                ...
                list(APPEND QT_EXTRA_PREFIX_DIRS "\"${ANDROID_SDK_DIR_arm64-v8a}/lib\"")
            endif()
            if(DEFINED ANDROID_SDK_DIR_x86)
                ...
                list(APPEND QT_EXTRA_PREFIX_DIRS "\"${ANDROID_SDK_DIR_x86}/lib\"")
            endif()
            if(DEFINED ANDROID_SDK_DIR_x86_64)
                ...
                list(APPEND QT_EXTRA_PREFIX_DIRS "\"${ANDROID_SDK_DIR_x86_64}/lib\"")
            endif()
            ...
            if (${QT_VERSION_MAJOR} EQUAL 6)
                if(EXISTS "${QT_HOST_PATH}/libexec/qmlimportscanner")
                    set(QML_IMPORT_SCANNER_FILE_PATH "${QT_HOST_PATH}/libexec/qmlimportscanner")
                elseif(EXISTS "${QT_HOST_PATH}/libexec/qmlimportscanner.exe")
                    set(QML_IMPORT_SCANNER_FILE_PATH "${QT_HOST_PATH}/libexec/qmlimportscanner.exe")
                elseif(EXISTS "${QT_HOST_PATH}/bin/qmlimportscanner")
                    set(QML_IMPORT_SCANNER_FILE_PATH "${QT_HOST_PATH}/bin/qmlimportscanner")
                elseif(EXISTS "${QT_HOST_PATH}/bin/qmlimportscanner.exe")
                    set(QML_IMPORT_SCANNER_FILE_PATH "${QT_HOST_PATH}/bin/qmlimportscanner.exe")
                else()
                    message(FATAL_ERROR "qmlimportscanner not found!")
                endif()
                ...
                elseif(EXISTS "${QT_HOST_PATH}/libexec/rcc.exe")
                    set(RCC_FILE_PATH "${QT_HOST_PATH}/libexec/rcc.exe")
                elseif(EXISTS "${QT_HOST_PATH}/bin/rcc")
                    set(RCC_FILE_PATH "${QT_HOST_PATH}/bin/rcc")
                elseif(EXISTS "${QT_HOST_PATH}/bin/rcc.exe")
                    set(RCC_FILE_PATH "${QT_HOST_PATH}/bin/rcc.exe")
                else()
                    message(FATAL_ERROR "rcc not found!")
                endif()
            ...
            message(STATUS "Using Android SDK build tool version ${ANDROID_BUILD_TOOLS_REVISION}")
            ...
            message(STATUS "Using androiddeployqt from ${ANDROID_DEPLOY_QT}")
            ...
            if (DEFINED KS_URL)
                ...
                if (DEFINED KS_PASS_FILE)
                    ...
                    message(STATUS "Using keystore ${KS_URL} to sign bundle/apk.")
                endif()
            else()
                set(ANDROID_DEPLOY_QT_CMD_ARGS "")
            endif()
            ...
            foreach(CURRENT_ANDROID_ABI ${ANDROID_ABIS})
                ...
                #message(STATUS "Creating project to build ${TARGET_NAME} for ${CURRENT_ANDROID_ABI}")
                string(REPLACE ";" "|" CMAKE_PREFIX_PATH_ "${CMAKE_PREFIX_PATH}")
                string(REPLACE ";" "|" ANDROID_ABIS_ "${ANDROID_ABIS}")
                string(REPLACE ";" "|" COMPONENTS_QT_ "${COMPONENTS_QT}")
                ExternalProject_Add(${TARGET_NAME}_${CURRENT_ANDROID_ABI}
                                    ...
                                    CMAKE_ARGS
                                        ...
                                        -DANDROID_ABIS=${ANDROID_ABIS_}
                                        -DQT_SDK_DIR=${ANDROID_SDK_DIR_${CURRENT_ANDROID_ABI}}
                                        -DQT_HOST_PATH=${QT_HOST_PATH}
                                        -DCMAKE_PREFIX_PATH=${CMAKE_PREFIX_PATH_}
                                        -DCMAKE_BUILD_TYPE=${CMAKE_BUILD_TYPE}
                                        -DANDROID_NATIVE_API_LEVEL=${ANDROID_NATIVE_API_LEVEL}
                                        -DTARGET_ARGN=${ARGN}
                                        -DAPP_BINARY_DIR=${CMAKE_CURRENT_BINARY_DIR}/${TARGET_NAME}/android-build/libs/${CURRENT_ANDROID_ABI}
                                        -DQT_VERSION_MAJOR=${QT_VERSION_MAJOR}
                                        -DCOMPONENTS_QT=${COMPONENTS_QT_}
                                        -DAndroidMultiAbiBuild=True)
            ...
            #message(STATUS "Cross-compiling for Android. Building for the following abi: ${ANDROID_ABI}.")
            add_library(${TARGET_NAME} SHARED ${TARGET_ARGN})
            set_target_properties(${TARGET_NAME} PROPERTIES APP_BINARY_DIR ${APP_BINARY_DIR})
            set(ANDROID_MACRO_armeabi-v7a ANDROID_ARMEABI_V7A)
            set(ANDROID_MACRO_arm64-v8a ANDROID_ARM64_V8A)
        ...
        if(${QT_VERSION_MAJOR} EQUAL 5)
            ...
            target_link_libraries(${TARGET_NAME} PRIVATE Qt${QT_VERSION_MAJOR}::AndroidExtras)
        endif()
    endfunction ()
    function (cute_add_library TARGET_NAME LIBRARY_TYPE)
        if (NOT "${ARGC}" EQUAL "2")
...
elseif (UNIX)
    if (IS_FIRST_RUN)
        message(STATUS "Build for UNIX host")
    endif()
    function (cute_add_executable TARGET_NAME)
        if (NOT "${ARGC}" EQUAL "1")
            message(FATAL_ERROR "Only the target name can be passed to cute_add_executable. Add sources through target_sources command.")
...
elseif (WIN32)
        if (IS_FIRST_RUN)
            message(STATUS "Build for Windows host")
        endif()
