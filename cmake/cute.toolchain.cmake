...
if(WIN32)
    string(REPLACE "\\" "/" QT_SDK_DIR "${QT_SDK_DIR}")
endif()
...
if (IS_FIRST_RUN)
    ...
    if (DEFINED ANDROID_TARGET)
        ...
        #message(STATUS "Configuring toolchain for Android build.")
        ...
        foreach(QT_ANDROID_DIR ${QT_ANDROID_DIRS})
            if (${QT_ANDROID_DIR} STREQUAL android)
                if (IS_FIRST_RUN)
                    #message(STATUS "Using ${QT_SDK_DIR}/android for all abis")
                endif()
        ...
        if(NOT DEFINED ANDROID_ABI)
            #message(STATUS "Setting ANDROID_ABI to ${MAIN_ANDROID_ABI}")
        ...
        #message(STATUS "Cross-compiling for Android ${ANDROID_ABI}")
    ...
    elseif (DEFINED APPLE_IOS_TARGET)
        ...
        save_var(TOOLCHAIN_TARGET_TYPE)
        message(STATUS "Configuring toolchain for Apple iOS build.")
        ...
        macro(cute_fetch_qt_plugins PLUGINS_DIR PLUGINS_NAMES_LIST_VAL PLUGINS_LIBS_LIST_VAL)
            if (NOT IS_DIRECTORY ${PLUGINS_DIR})
                ...
                message(STATUS "Fetching Qt plugins located at ${PLUGINS_DIR}.")
            endif()
        ...
        if (NOT IS_DIRECTORY ${QT_SDK_DIR}/plugins)
            message(FATAL_ERROR "Qt plugins directory ${QT_SDK_DIR}/plugins does not exist.")
        #elseif (NOT IS_DIRECTORY ${QT_SDK_DIR}/qml)
            #message(FATAL_ERROR "Qt qml directory ${QT_SDK_DIR}/qml does not exist.")
        endif()
    ...
    if (DEFINED WASM_TARGET)
        ...
        save_var(TOOLCHAIN_TARGET_TYPE)
        #message(STATUS "Configuring toolchain for WebAssembly.")
        if (NOT DEFINED QT_SDK_DIR)
            message(FATAL_ERROR "QT_SDK_DIR must be defined as the directory containing Qt's iOS installation directory.")
        else()
            save_var(QT_SDK_DIR)
        ...
        if (NOT EXISTS "${NM_EXE}" AND NOT EXISTS "${NM_EXE}.exe")
            ...
            foreach(current_entry ${CUTE_QT_PLUGINS})
                ...
                if (NOT ("${PLUGIN_LIBRARY}" STREQUAL ""))
                    ...
                    if (NOT ("${MANGLED_PLUGIN_FUNCTION_NAMES}" STREQUAL ""))
                        string (REPLACE "\n" ";" MANGLED_PLUGIN_FUNCTION_NAMES "${MANGLED_PLUGIN_FUNCTION_NAMES}")
                        foreach(mangled_name ${MANGLED_PLUGIN_FUNCTION_NAMES})
                            if(NOT "${mangled_name}" MATCHES "qt_static_plugin")
                                continue()
                            endif()
                            execute_process(COMMAND
    ...
    #message(STATUS "Detecting toolchain target.")
    ...
    elseif("${TOOLCHAIN_TARGET_TYPE}" STREQUAL "APPLE_IOS_TARGET")
        #message(STATUS "Apple iOS target.")
...
if (ANDROID_TARGET)
    ...
    #message(STATUS "Loading ${ANDROID_NDK}/build/cmake/android.toolchain.cmake")
    include(${ANDROID_NDK}/build/cmake/android.toolchain.cmake)
elseif (APPLE_IOS_TARGET)
    if (APPLE_IOS_DEVICE)
        message(STATUS "Cross-compiling for Apple iOS devices")
    elseif (APPLE_IOS_SIMULATOR)
        message(STATUS "Cross-compiling for Apple iOS simulator")
    endif()
    #
    ...
    if (IS_FIRST_RUN)
        #message(STATUS "Cross Compiling for Apple iOS")
    ...
    if ("${MIN_IOS_SDK_TARGET}" STREQUAL "")
        message(STATUS "MIN_IOS_SDK_TARGET was not defined. Querying XCode for available SDK version...")
        execute_process(COMMAND xcodebuild -sdk ${SDKROOT} -version SDKVersion
                        OUTPUT_VARIABLE MIN_IOS_SDK_TARGET
                        ERROR_QUIET
                        OUTPUT_STRIP_TRAILING_WHITESPACE)
    endif()
    message(STATUS "Set minimum iOS SDK version to ${MIN_IOS_SDK_TARGET}")
...
elseif (WASM_TARGET)
    #message(STATUS "Cross-compiling for WebAssembly")
    ...
    endforeach ()
    #message(STATUS "Loading ${WASM_SDK}/cmake/Modules/Platform/Emscripten.cmake")
    include(${WASM_SDK}/emscripten/cmake/Modules/Platform/Emscripten.cmake)
    #
    # Setting linker flags
    #
    set(EMCC_COMMON_LFLAGS "-flto -Oz -s WASM=1 -s FULL_ES2=1 -s USE_WEBGL2=1 -s MIN_WEBGL_VERSION=2 ")
    set(EMCC_COMMON_LFLAGS "${EMCC_COMMON_LFLAGS} -s INITIAL_MEMORY=64MB -s ALLOW_MEMORY_GROWTH=1 ")
    set(EMCC_COMMON_LFLAGS "${EMCC_COMMON_LFLAGS} -s ERROR_ON_UNDEFINED_SYMBOLS=1 -lidbfs.js -lembind ")
    set(EMCC_COMMON_LFLAGS "${EMCC_COMMON_LFLAGS} -s ASYNCIFY=1 -s EXPORT_NAME=application ")
    set(EMCC_COMMON_LFLAGS "${EMCC_COMMON_LFLAGS} -s EXPORTED_RUNTIME_METHODS=ccall,cwrap,specialHTMLTargets,UTF16ToString,stringToUTF16,JSEvents,specialHTMLTargets,FS,callMain")
